...
class PrincipalView:
    def __init__(self, id_usuario, nome):
        ...
        tk.Label(frame, text="Ingredientes:", bg="white").grid(row=2, column=0)
        self.ingr_r = tk.Text(frame, width=30, height=5)
        self.ingr_r.grid(row=2, column=1)

        tk.Label(frame, text="Modo de Preparo:", bg="white").grid(row=3, column=0)
        self.modo_r = tk.Text(frame, width=30, height=5)
        self.modo_r.grid(row=3, column=1)

        tk.Label(frame, text="Categoria:", bg="white").grid(row=4, column=0)
        self.categoria = tk.Entry(frame, width=35)
        self.categoria.grid(row=4, column=1)

        tk.Label(frame, text="Dificuldade:", bg="white").grid(row=5, column=0)
        self.combo_dificuldade = ttk.Combobox(frame, values=["Fácil", "Médio", "Difícil"], state="readonly", width=10)
        self.combo_dificuldade.grid(row=5, column=1)

        tk.Label(frame, text="Link da Imagem:", bg="white").grid(row=6, column=0)
        self.link_imagem = tk.Entry(frame, width=35)
        self.link_imagem.grid(row=6, column=1)

        tk.Label(frame, text="Link do Vídeo:", bg="white").grid(row=7, column=0)
        self.link_video = tk.Entry(frame, width=35)
        self.link_video.grid(row=7, column=1)

        self.lista = tk.Listbox(frame, width=40, height=8)
        self.lista.grid(row=8, column=1, pady=10)

        tk.Button(frame, text="Cadastrar Receita", bg="#4CAF50", fg="white", command=self.cadastrar).grid(row=9, column=0, columnspan=2, pady=5)
        tk.Button(frame, text="Ver Receita", bg="#2196F3", fg="white", command=self.ver_receita).grid(row=10, column=0, columnspan=2, pady=5)
        tk.Button(frame, text="Gerar Cardápio", bg="#9C27B0", fg="white", command=self.cardapio).grid(row=11, column=0, columnspan=2, pady=5)
